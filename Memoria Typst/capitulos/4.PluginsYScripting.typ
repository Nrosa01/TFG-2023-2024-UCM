...
Esto demuestra la diferencia que puede llegar a haber entre ambas aproximaciones a un sistema modular que aproveche la CPU. Como el objetivo es recrear el sistema de partículas de Noita explorando todas las posibilidades de la CPU, se implementarán los sistemas de extensión mediante librerías dinámicas y lenguajes de scripting. Tras esto, se valorán ambas opciones, comparándolas con su alterntiva en GPU para así poder determinar cuál es la mejor opción para este caso en concreto.
